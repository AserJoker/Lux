...
protected:
  Image()
      : _pImage(nullptr), _srcRect({0, 0, 0, 0}), _dstRect({0, 0, 0, 0}),
        _ptCenter({0, 0}), _lfAngle(0), _flip(SDL_FLIP_NONE), _blAbsolute(true),
        _nZIndex(0) {}

public:
  core::Pointer<resource::Image> getImage() { return _pImage; }
  void setImage(const core::Pointer<resource::Image> &image) {
    _pImage = image;
  }
  SDL_Rect &getSrcRect() { return _srcRect; }
  SDL_Rect &getDstRect() { return _dstRect; }
  SDL_Point &getCenter() { return _ptCenter; }
  double &getAngle() { return _lfAngle; }
  bool &IsAbsolute() { return _blAbsolute; }
  int &zIndex() { return _nZIndex; }
