...
private:
  std::map<std::string, std::vector<BaseEventEmitter *>> _listeners;
...
public:
  ...
  template <class T> class EventEmitter : public BaseEventEmitter {
  public:
  ...
  template <class T> void emit(T *event) {
    ...
    for (auto &_lis : listeners) {
      auto lis = (EventEmitter<T> *)_lis;
      lis->on(event);
    }
  }
  void addEventListener(const std::string &event, BaseEventEmitter *lis) {
  ...
  void removeEventListener(const std::string &event, BaseEventEmitter *lis) {
    auto list = _listeners[event];
    std::erase(list, lis);
    _listeners[event] = list;
  }
