...
namespace lux::core {
    ...
    public:
        template<class T>
        static void provide(std::string token = T::TOKEN, TYPE type = SINGLETON) {
            _providers[token] = {
                    type,
                    []() -> Pointer<Object> { return Pointer<Object>((Object *) new T()); },
                    Pointer<Object>(nullptr)};
        }

        template<class T>
        static Pointer<T> inject(std::string token = T::TOKEN) {
...
#define PROVIDE_PROTOTYPE(provider) lux::core::Container::provide<provider>(provider::TOKEN,core::Container::PROTOTYPE)
#define INJECT(provider) lux::core::Container::inject<provider>()
#define PROVIDE_AND_INJECT(provider)                                           \
  PROVIDE(provider);                                                           \
  INJECT(provider);
#endif