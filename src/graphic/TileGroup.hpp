...
public:
  ...
  TileGroup()
      : _pData(nullptr), _ppData(nullptr), _pTile(nullptr), _isLock(false) {}
  ~TileGroup() override {
    if (_pData) {
      delete[] _pData;
      _pData = nullptr;
  ...
  static core::Pointer<TileGroup> create(core::Pointer<Tile> tile, int width,
                                         int height,
                                         const int *data = nullptr) {
    auto group = INJECT(TileGroup);
    Sprite::create(width * tile->getTileWidth(), height * tile->getTileHeight(),
                   group.cast<Sprite>());
    group->_pTile = tile;
